...
COLOR_MAP = {
    0: [255, 100, 100],  # Sky (RGB: Blue)
    1: [34, 139, 34],    # Trees (RGB: Forest Green)
    2: [0, 255, 127],    # Bushes (RGB: Spring Green)
    3: [173, 255, 47],   # Grass (RGB: Green Yellow)
    4: [139, 69, 19]     # Rocks (RGB: Saddle Brown)
}
...
def generate_synthetic_data(num_samples, split='train'):
    """Generate simple synthetic offroad images and corresponding semantic masks."""
    ...
    # If test set, the dir structure is slightly different
    if split == 'testImages':
        img_dir = 'data/testImages'
        
    for i in range(num_samples):
        # Create a base blank image (RGB) and mask (1-channel)
        H, W = 256, 256
        image = np.zeros((H, W, 3), dtype=np.uint8)
        mask = np.zeros((H, W), dtype=np.uint8)
        
        # 1. Sky (background top half)
        image[:H//2, :, :] = [255, 140, 100]  # Blue sky in BGR (cv2 uses BGR for save, but let's keep it RGB and convert on save)
        image[:H//2, :, :] = [100, 100, 250]  # RGB: light blue sky
        mask[:H//2, :] = 0
        
        # 3. Grass (background bottom half)
        image[H//2:, :, :] = [100, 200, 50]   # RGB: base grass
        mask[H//2:, :] = 3
        
        # Add random elements to simulate noise and texture
        # 4. Rocks
        for _ in range(np.random.randint(2, 6)):
            cx, cy = np.random.randint(0, W), np.random.randint(H//2 + 20, H)
            r = np.random.randint(10, 30)
            cv2.circle(image, (cx, cy), r, (120, 120, 120), -1) # RGB Gray
            cv2.circle(mask, (cx, cy), r, 4, -1)                # Class 4
            
        # 1. Trees (Vertical rectangles in background)
        for _ in range(np.random.randint(1, 4)):
            tx = np.random.randint(20, W-20)
            ty = np.random.randint(H//3, H//2)
            tw, th = np.random.randint(10, 30), np.random.randint(60, 100)
            cv2.rectangle(image, (tx-tw//2, ty), (tx+tw//2, ty+th), (30, 100, 30), -1) # Dark green
            cv2.rectangle(mask, (tx-tw//2, ty), (tx+tw//2, ty+th), 1, -1)              # Class 1
            
        # 2. Bushes (Circles on the horizon)
        for _ in range(np.random.randint(2, 5)):
            cx, cy = np.random.randint(0, W), np.random.randint(H//2 - 10, H//2 + 20)
            r = np.random.randint(15, 35)
            cv2.circle(image, (cx, cy), r, (20, 160, 60), -1)   # Brighter green
            cv2.circle(mask, (cx, cy), r, 2, -1)                # Class 2
...
def apply_color_map(mask, num_classes=5):
